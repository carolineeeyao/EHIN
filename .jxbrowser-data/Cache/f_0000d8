...
Lexer.prototype.token = function(src, top, bq) {
  ...
  while (src) {
    ...
    // Tab-pane
    if (cap = this.rules.tab.exec(src)) {
      src = src.substring(cap[0].length);

      var content = cap[2];
      // For some reason doesn't give the sub-groups
      var panesInput = content.match(/[\r\n]+\|{2}(.*?)\|{2}([\s\S]+?(?=[\r\n]+\|{2}|$))/g);

      panes = [];
      titles = [];

      for (i = 0; i < panesInput.length; i++) {
        var p = panesInput[i].match(/[\r\n]+\|{2}(.*?)\|{2}([\s\S]+?(?=[\r\n]+\|{2}|$))/);
        var title = p[1];
        var body = p[2];
        panes.push({title: title, body: body});
      ...
      // Render contents of tabs
      for (i = 0; i < panes.length; i++) {
        this.tokens.push({
          type: 'tab_pane_start',
        });

        this.token(panes[i].body, top, true);
...
Renderer.prototype.tab = function(titles, panes) {
  tabGroupNumber += 1;
  var ret = '<div class="panel panel-default">' +
   ...
   for (i = 0; i < titles.length; i++) {
      ret += '<li'+active+'><a data-toggle="tab" href="#tab'+tabGroupNumber+'-'+i+'">'+titles[i]+'</a></li>'
      active = '';
   }
   ret += '</ul>'
   ...
   active = 'in active'
   for (i = 0; i < panes.length; i++) {
      ret += '<div id="tab'+tabGroupNumber+'-'+i+'" class="tab-pane '+active+'">';
      ret += panes[i];
      ret += '</div>'
      active = '';
   }
...
Renderer.prototype.image = function(href, title, text) {
  var out = '<div class="modal-pop" style="cursor:zoom-in;"><img src="' + href + '" alt="' + text + '" class="img-responsive"';
  if (title) {
    out += ' title="' + title + '"';
  }
...
Parser.prototype.tok = function() {
  switch (this.token.type) {
    ...
    case 'tab_start': {
      var titles = this.token.titles;
      var panes = [];

      while (this.next().type !== 'tab_end'){
        var pane = '';
        while (this.next().type !== 'tab_pane_end')
          pane += this.tok();
        panes.push(pane);
      }

      return this.renderer.tab(titles, panes);
    }
    case 'tooltip_start': {
      var color = '';
      var title = '';
